...
function AdminBrandList() {
  const { brands, isSearched } = useShopContext();

  if (brands.length === 0 && isSearched) {
    return (
    ...
    return (
      <div className="text-center py-12">
        <div className="text-gray-500 text-lg">
          ...
          <p className="text-gray-500">Comece adicionando sua primeira marca</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      {brands.map((brand) => (
        <AdminBrandCard key={brand.id} brand={brand} />
      ))}
    </>
  );
}
